...
"""
    ...
    from src.dataset_gpt import MyDataset
    ...
    class LitGPT(pl.LightningModule):
        ...
        def configure_optimizers(self):
            ...
            param_groups = [{"params": self.parameters(), "my_lr_scale": 1.0}]
    ...
    class GPTTrainCallback(Callback):
        def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
            # Example: Log the loss
            loss = outputs['loss'] if isinstance(outputs, dict) else outputs
            if hasattr(trainer, "my_loss_sum"):
                trainer.my_loss_sum += loss.item()
            # trainer.logger.experiment.add_scalar("train_loss", loss.item(), trainer.global_step)
    
    print(f"#### length of args.load_model: {len(args.load_model)}")
    print(f"#### args.my_pile_stage: {args.my_pile_stage}")

    if len(args.load_model) == 0 or args.my_pile_stage == 1:  # shall we build the initial weights?
        init_weight_name = f"{args.proj_dir}/rwkv-init.pth"
        # generate_init_weight(model, init_weight_name)  # save initial weights
        model.init_weights()
        torch.save(model.state_dict(), init_weight_name)
    ...
    assert args.accelerator == "gpu", "DeepSpeed requires GPU. Please set args.accelerator='gpu'."
    assert torch.cuda.is_available(), "CUDA is not available. Ensure GPUs are accessible."

    if pl.__version__[0]=='2':
        print("#### pl version = 2 ####")
        trainer = Trainer(accelerator=args.accelerator,strategy=args.strategy,devices=args.devices,num_nodes=args.num_nodes,precision=args.precision,
        logger=args.logger,callbacks=[GPTTrainCallback()],max_epochs=args.max_epochs,check_val_every_n_epoch=args.check_val_every_n_epoch,num_sanity_val_steps=args.num_sanity_val_steps,
    ...
    data_loader = DataLoader(train_data, shuffle=False, pin_memory=True, batch_size=args.micro_bsz, num_workers=1, persistent_workers=False, drop_last=True, collate_fn=collate_fn)
